...
def cli_provision_assets(
  ...
  """Download the required dataset and models for a task."""
  provision_assets(task, base_url, destination, models, chunk)


def provision_assets(task, base_url, destination, models, chunk) -> None:
  sized_prefixes = get_sized_prefixes(base_url, task, models)
  dl_small = Downloader(
    max_conn=64,
    max_splits=1,
    progress=True,
...
def get_sized_prefixes(base_url: str, task: str, models: bool) -> List[Tuple[int, str]]:
  ...
  for raw in r.text.splitlines():
    ...
    if c == task and (k == "datasets" or (models and k == "models")):
